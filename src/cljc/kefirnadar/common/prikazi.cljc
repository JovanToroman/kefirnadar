...
(defn pocetna-strana []
  (let [naslov-dugmeta-trazim "Nađi kulturu"
        naslov-dugmeta-delim "Podeli kulturu"]
    [:div.d-flex.flex-column.justify-content-center.align-items-center.pt-5 {:style {:min-height "80vh"}}
     [:div.row.flex-lg-row-reverse.align-items-center.g-5
      [:div.col-10.col-sm-8.col-lg-6
       [:img.d-block.mx-lg-auto.img-fluid {:src "/images/heroj.avif" :alt "srbija-širi-kefir" :width "700"
      ...
      [:div.col-lg-6 [:h1.display-5.fw-bold.lh-1.mb-3 "Dobrodošli u Kefir na dar, aplikaciju za
       ...
       [:p.lead (str "Kefir na dar je jedina aplikacija u Srbiji i regionu za deljenje zrnaca mlečnog i vodenog kefira"
                  " i kombuhe. Ova mrežna aplikacija funkcioniše tako što korisnici koji imaju višak zrnaca postave oglas u kojem"
                  " navedu koju vrstu zrnaca dele i gde se nalaze, kao i na koji način dele (lično ili poštom)."
                  " Da biste videli listu svih oglasa, pritisnite dugme '" naslov-dugmeta-trazim "'. Ako pak želite "
       ...
       [:div.mt-3
        ...
        [:div "Ako želite da sačuvate oglase koje postavite tako da im možete pristupiti kasnije, predlažemo da se "
         [:a {:href "/registracija"} "registrujete"] "."]]]]]))
